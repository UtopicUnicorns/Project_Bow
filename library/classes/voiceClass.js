...
class voiceConstruct {
  ...
  async connect(info) {   
    if (!this.player[info.guild]) this.player[info.guild] = lamp.players.get(info.guild);
      if(!this.playerDeploy[info.guild]) this.playerDeploy[info.guild] = { songs: [], state: null, playing: null , settings: { timescale: {pitch: 1.0}, volume: 0.5 }, channel: info.command };
